...
StellaEnvironment::StellaEnvironment(OSystem* osystem, RomSettings* settings)
  ...
  Logger::Info << "Random seed is "
               << m_osystem->settings().getInt("random_seed") << std::endl;
  // Initialize RNG
  if (m_osystem->settings().getInt("random_seed") == 0) {
    m_random.seed((uint32_t)time(NULL));
  } else {
    int seed = m_osystem->settings().getInt("random_seed");
    assert(seed >= 0);
...
ALEState StellaEnvironment::cloneState() {
  return m_state.save(m_osystem, m_settings, &m_random, m_cartridge_md5);
}
